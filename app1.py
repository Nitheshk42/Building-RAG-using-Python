...
#                 # ==================== NEW: RETRIEVAL DEMO ====================
#                                 # ==================== FIXED RETRIEVAL DEMO ====================
#                 st.subheader("🔥 Live Demo: How Retrieval Works Using HNSW")

#                 st.write("Test how HNSW finds relevant chunks for your question.")
...
#                         st.warning("⚠️ Please run 'Start Full Ingestion' first!")
#                     else:
#                         with st.spinner("HNSW is searching for similar chunks..."):
#                             retriever = st.session_state.vectorstore.as_retriever(
#                                 search_kwargs={"k": 3}
#                             )
#                             retrieved_docs = retriever.invoke(test_question)

#                             st.success(f"✅ HNSW retrieved **{len(retrieved_docs)}** most relevant chunks!")

#                             for i, doc in enumerate(retrieved_docs, 1):
#                                 with st.expander(f"📌 Relevant Chunk {i}"):
#                                     content = doc.page_content[:700] + "..." if len(doc.page_content) > 700 else doc.page_content
#                                     st.write(content)
#                                     st.caption("This chunk was chosen because it has the highest similarity to your question.")

#                 st.info("""
#                 **How This Works Behind the Scenes:**
#                 1. Your question is converted to a vector
#                 2. HNSW Index quickly finds closest vectors (using highways + small roads)
#                 3. Returns only the most relevant chunks
#                 4. These chunks will be sent to LLM in the next stage
#                 """)
    ...
    if not os.path.exists("data") or len(os.listdir("data")) == 0:
        ...
        with st.spinner("Running full pipeline..."):
            try:
                ...
                vectorstore = create_vector_store(splits, embeddings)
                st.session_state.vectorstore = vectorstore   # Save for retrieval
...
# ===================== FULL RETRIEVAL DEMO (Outside button) =====================
st.subheader("🔥 Live Demo: How Retrieval Works with HNSW")
...
if st.button("🔍 Retrieve Relevant Chunks"):
    if 'vectorstore' not in st.session_state or st.session_state.vectorstore is None:
        st.error("⚠️ Please run 'Start Full Ingestion' first!")
    else:
        with st.spinner("🔍 HNSW is searching for relevant chunks..."):
            retriever = st.session_state.vectorstore.as_retriever(
                search_kwargs={"k": 4}
            )
            retrieved_docs = retriever.invoke(test_question)
            ...
            for i, doc in enumerate(retrieved_docs, 1):
                with st.expander(f"📌 Relevant Chunk {i}"):
                    ...
                    st.write(content[:750] + "..." if len(content) > 750 else content)
                    st.caption("This chunk was selected because it has the highest similarity to your question.")

st.info("""
**How Retrieval Works:**
1. Question → Converted to embedding vector
2. HNSW quickly finds closest vectors using its index
3. Returns only the most relevant chunks (not all)
4. These chunks will be used to generate answer in next stage (Day 2)
""")